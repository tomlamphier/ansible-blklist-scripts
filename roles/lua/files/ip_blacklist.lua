-- a quick LUA access script for nginx to check IP addresses against 
-- key/value pairs in Redis, and if a match is found send a HTTP 403.
--
-- allows for a common blacklist to be shared between a bunch of nginx
-- web servers using a remote redis instance. lookups are cached for a
-- configurable period of time.
--
-- block an ip for 3 days (259200 sec):
--   redis-cli SETEX 10.1.1.1 259200 T
-- remove an ip:
--   redis-cli DEL ip_blacklist 10.1.1.1
--
...
-- ngx.log(ngx.DEBUG, "ip_blacklist hello")
...
local result, err = redis:exists(ip)
if not result then
